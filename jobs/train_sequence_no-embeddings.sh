...
srun python3 -u run.py --device cuda --seed 13 --eval-freq 200 --configuration sequence-to-char --learning-rate 1e-4 --metric-types jaccard-similarity levenshtein-distance --language english --challenge post-ocr-correction --batch-size 256 --hidden-dimension 512 --encoder-embedding-size 128 --decoder-embedding-size 128 --dropout 0.2 --number-of-layers 1 --enable-external-logging --pretrained-weights bert-base-cased --max-training-minutes 4320 --include-pretrained-model --pretrained-model-size 768 --pretrained-max-length 512  > output/seq-bert-13.txt

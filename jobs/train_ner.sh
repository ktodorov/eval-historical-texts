...
echo "srun python3 -u run.py --device cuda --eval-freq 200 --seed 13 --patience 150 --epochs 3000 --configuration rnn-simple --learning-rate 1e-2 --metric-types f1-score precision-score recall-score --language $LANGUAGE --challenge named-entity-recognition --batch-size 128 --enable-external-logging --pretrained-weights $PRETRAINEDWEIGHTS --hidden-dimension 256 --embeddings-size 64 --dropout 0.5 --number-of-layers 1 --label-type $LABELTYPE --reset-training-on-early-stop --training-reset-epoch-limit 5 $INCLUDEPRETR --pretrained-model-size 768 --pretrained-max-length 512 --learn-new-embeddings --checkpoint-name $LANGUAGE-$LABELTYPE$FT-$PRETRAINEDMODEL$PRETR $NOATTENTION --bidirectional-rnn $FASTTEXT --fasttext-model $FASTTEXTMODEL --fasttext-model-size 300 --max-training-minutes 4300 --merge-subwords --learn-character-embeddings --character-embeddings-size 32 --replace-all-numbers --pretrained-model $PRETRAINEDMODEL"

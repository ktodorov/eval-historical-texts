...
srun python3 -u run.py --device cuda --eval-freq 100 --seed 13 --configuration rnn-simple --learning-rate 1e-4 --metric-types f1-score --language $LANGUAGE --challenge named-entity-recognition --batch-size 128 --enable-external-logging --pretrained-weights bert-base-multilingual-cased --hidden-dimension 256 --embeddings-size 64 --label-type $LABELTYPE --reset-training-on-early-stop --training-reset-epoch-limit 5 --learn-new-embeddings --checkpoint-name $LANGUAGE-$LABELTYPE-scratch > output/ner-scratch-$LANGUAGE-$LABELTYPE-13.txt

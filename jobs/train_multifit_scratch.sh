...
srun python3 -u run.py --device cuda --seed 13 --configuration multi-fit --learning-rate 1e-3 --metric-types jaccard-similarity levenshtein-distance --language english --challenge ocr --batch-size 4 --hidden-dimension 256 --sentence-piece-vocabulary-size 30522 --embedding-size 64 --dropout 0 --number-of-layers 1 --enable-external-logging --pretrained-weights bert-base-cased --max-training-minutes 4320 --learn-encoder-embeddings --validation-dataset-reduction-size 0.020 > output/multifit-13.txt

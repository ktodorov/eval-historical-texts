...
srun python3 -u run.py --device cuda --seed 13 --eval-freq 600 --patience 100 --configuration kbert --learning-rate 1e-5 --language $LANGUAGE --corpus $CORPUS --challenge semantic-change --batch-size 4 --pretrained-weights bert-base-cased --pretrained-vocabulary-size 30522 --max-training-minutes 1440 --enable-external-logging --skip-validation > output/semeval-$LANGUAGE-$CORPUS-13.txt

...
class NELine:
    ...
    def add_data(self, csv_row: dict):
        ...
        self._add_entity_if_available(csv_row, 'MISC', self.misc, use_none_if_empty=True)
        ...
        self._add_entity_if_available(csv_row, 'NE-FINE-LIT', self.ne_fine_lit, use_none_if_empty=True)
        self._add_entity_if_available(
        ...
        self._add_entity_if_available(csv_row, 'NE-NESTED', self.ne_nested, use_none_if_empty=True)
        self._add_entity_if_available(csv_row, 'NEL-LIT', self.nel_lit, use_none_if_empty=True)
        self._add_entity_if_available(csv_row, 'NEL-METO', self.nel_meto, use_none_if_empty=True)

    def tokenize_text(self, tokenizer_service: TokenizerService):
        text = self.get_text()
        token_ids, encoded_tokens = tokenizer_service.encode_string(text)
        ...
        if len(encoded_tokens) > len(self.tokens):
            ...
            for i, token in enumerate(self.tokens):
                while corresponding_counter < len(encoded_tokens) and encoded_tokens[corresponding_counter].startswith('##'):
                    # we copy the value of the original token
                    new_misc.insert(corresponding_counter, self.misc[i])
                    new_ne_coarse_lit.insert(corresponding_counter, self.ne_coarse_lit[i])
                    new_ne_coarse_meto.insert(corresponding_counter, self.ne_coarse_meto[i])
                    new_ne_fine_lit.insert(corresponding_counter, self.ne_fine_lit[i])
                    new_ne_fine_meto.insert(corresponding_counter, self.ne_fine_meto[i])
                    new_ne_fine_comp.insert(corresponding_counter, self.ne_fine_comp[i])
                    new_ne_nested.insert(corresponding_counter, self.ne_nested[i])
                    new_nel_lit.insert(corresponding_counter, self.nel_lit[i])
                    new_nel_meto.insert(corresponding_counter, self.nel_meto[i])
        ...
        assert len(token_ids) == len(self.tokens)

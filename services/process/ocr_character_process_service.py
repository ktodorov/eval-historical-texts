...
class OCRCharacterProcessService(ProcessServiceBase):
    ...
    def get_language_data(self, run_type: RunType):
        ...
        if run_type == RunType.Train:
            train_language_data_path = self._get_language_data_path(RunType.Train)
            language_data = self._load_language_data(
                train_language_data_path,
                RunType.Train,
                self._arguments_service.train_dataset_limit_size)

        elif run_type == RunType.Validation:
            dev_language_data_path = self._get_language_data_path(
                RunType.Validation)
            language_data = self._load_language_data(
                dev_language_data_path,
                RunType.Validation,
                self._arguments_service.validation_dataset_limit_size)

            self._log_dev_data_statistics(language_data)

        return language_data

    def _log_dev_data_statistics(self, language_data):
        edit_distances = []
